...
type CommonResp = {
  ...
  f2_name: string;
  count: number;
  common: { opponent: string; f1: FightResult; f2: FightResult }[];
};
...
export default function CommonOpponents({
  ...
  const [data, setData] = useState<CommonResp | null>(null);
  ...
  // short last names for the result lines
  const f1Last = data.f1_name.split(" ").pop() ?? data.f1_name;
  const f2Last = data.f2_name.split(" ").pop() ?? data.f2_name;
